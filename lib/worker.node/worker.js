...
var Worker = (function (_EventEmitter) {
  ...
  Worker.prototype.handleMessage = function handleMessage(message) {
    if (message.error) {
      ...
      this.emit('message', { data: resultBuffer, args: [].concat(this.message.response) });
      this.emit('done', { data: resultBuffer, args: [].concat(this.message.response) }); // this one is just for convenience
    }
  };
